...
window.alert = function(message, backgroundColor, textColor, borderSize, borderbackgroundColor) {
	/* var rgb = [0,0,0];
	var chosenbackgroundColor; */
	...
	if (borderbackgroundColor == null) { borderbackgroundColor = "black" }
	/* function convRGB(i){
    var c = (i & 0x00FFFFFF)
        .toString(16)
        .toUpperCase();
	...
	hexToRgb(convRGB(getHash(backgroundColor)));
var o = Math.round(((parseInt(rgb[0]) * 299) + (parseInt(rgb[1]) * 587) + (parseInt(rgb[2]) * 114)) /1000);
    if(o > 125) {
        chosenbackgroundColor = `black`;
    }else{ 
        chosenbackgroundColor = `white`;
    } */
document.getElementById(`newAlert`).innerHTML = `<div class="alert" style="border:`+borderSize+`px solid `+borderbackgroundColor+`; border-pixel-size: 10px; opacity: 0.99;transition: opacity 0.6s;padding: 20px;background-backgroundColor:`+backgroundColor+`;backgroundColor: `+textColor+`;margin-bottom: 15px;margin-left: 30%;margin-right: 30%;">
